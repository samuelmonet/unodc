...
def indicateur(domaine, ticktext, tickval, title, progression_temps, progression_objectif, couleur,step=None):
    gauge={'shape': "bullet",
    ...
    if step is not None:
        gauge['steps']=[{'range': [step,progression_objectif], 'color': "gray"}]
...
layout = html.Div([html.Div(dcc.Graph(figure=fig), style={"border-style": "solid"}),
                       ...
                       # Proj/Obj completion
                       html.Div([
                           html.Div([html.H5(id='outcome1', children='Reinforcement of the capacities of Maritime '
                                     ...
                                     html.H5(id='outcome2', children='Identification of obstacles hampering the '
                           ...
                           html.Div([html.H2(id='text_map', style={"text-align": "center"}),
                                     ...
                                     html.Div([
                                         html.Div([
                                             html.H5(id='outcome3', children='Improved capacity building on Port State '
                                         ...
                                         html.Div([
                                             html.H5(id='outcome4', children='Implementation of ISPS Code and related '
                                         ...
                                         html.Div([
                                             html.H5(id='outcome6', children='Implementation of a cargo targeting '
                           ...
                           html.Div([html.H5(id='outcome5', children='Develop and support the implementation of Port '
...
def update_graphs(obj, clicks, what):
    ...
    if what is not None:
        ...
        old_c = old[old['Country'] == country].copy()
        overall_progression = country_coord['all'].values[0] /16
        old_progression=old_c['all'].values[0] / 16
        fig2 = indicateur([0, 100], ['0', '25%', '50%', '75%', '100%'], [0, 25, 50, 75, 00],
                          f'Project overall completion in {country}', progression,
                        overall_progression, retard(progression, overall_progression),step=old_progression)
        #print(country)
        #print(country_coord)
        ...
        if obj == 'all':
            ...
            old_progress = old_c[obj].values[0]
            ...
            for i in range(16):
                if position_pays[i] == country:
                    ...
                    outputs_fig[i] = indicateur(outputs[obj]['range'], outputs[obj]['ticktext'],
                                                ...
                                                progression, progress, retard(progression, progress),step=old_progress)
    ...
    # indicateur(range, ticktext, tickval, title, progression_temps, progression_objectif, couleur)
        ...
        if obj == 'all':
            ...
            old_progression = old[old['Country'] == 'All']['all'].values[0] / 144
            fig2 = indicateur([0, 100], ['0', '25%', '50%', '75%', '100%'], [0, 25, 50, 75, 100],
                              "Project's' overall completion (mean completion of all outputs)",
                              progression, overall_progression, retard(progression, overall_progression),step=old_progression)
            for i in range(16):
                indic = country_coord.columns.tolist()[1:-1][i]
                progress = country_coord[indic].values[0]/9
                old_progress=old_c[indic].values[0]/9
                outputs_fig[i] = indicateur([0, 100], ['0', '25%', '50%', '75%', '100%'], [0, 25, 50, 75, 100],
                                          indic, progression, progress, retard(progression, progress),step=old_progress)
            ...
            old_progression = old[old['Country'] == 'All'][obj].values[0] / 800
            fig2 = indicateur([0, 100], ['0', '25%', '50%', '75%', '100%'], [0, 25, 50, 75, 100],
            ...
            for i in range(16):
                if position_pays[i] != '':
                    ...
                    old_progress = old[old['Country'] == pays][obj].values[0]
                    outputs_fig[i] = indicateur([0, 100], ['0', '25%', '50%', '75%', '100%'], [0, 25, 50, 75, 100],
                                                pays, progression, progress, retard(progression, progress),step=old_progress)
    ...
    if obj=='all':
        zmax=countries[obj].max()/16
        zmin = countries[obj].min() / 16
        z=countries[obj]/16
    else:
        zmax = countries[obj].max()
        zmin = countries[obj].min()
        z=countries[obj]
    ...
    carte2 = go.Figure(go.Choroplethmapbox(
        ...
        z=z,  # importing column consisting of
        zmin=zmin, zmax=zmax,  # active cases data
